...
std::map<std::string, std::string> IMAGINGMODULESSHARED_EXPORT   BBLogNorm::GetParameters()
{
    ...
    parameters["BBroi"]                = kipl::strings::Vector2String(BBroi);

    parameters["doseBBroi"]            = kipl::strings::Vector2String(doseBBroi);
...
void IMAGINGMODULESSHARED_EXPORT  BBLogNorm::LoadReferenceImages(const std::vector<size_t> &roi)
{
    ...
    if (bUseExternalBB && nBBextCount!=0)
    {
        ...
        {
            msg.str("");
            msg<<"Failed to load external BB data. ImagingException "<<e.what();
            throw ImagingException(msg.str(),__FILE__,__LINE__);
        }
        catch (ReaderException &e)
        {
            msg.str("");
            msg<<"Failed to load external BB data. ReaderException "<<e.what();
            throw ImagingException(msg.str(),__FILE__,__LINE__);
        }
        catch (kipl::base::KiplException &e)
        {
            msg.str("");
            msg<<"Failed to load external BB data. KiplException "<<e.what();
            throw ImagingException(msg.str(),__FILE__,__LINE__);
        }
        catch (std::exception &e)
        {
            msg.str("");
            msg<<"Failed to load external BB data. STL Exception "<<e.what();
            throw ImagingException(msg.str(),__FILE__,__LINE__);
        }
        catch (...)
        {
...
//                          std::cout << "doselist: " << std::endl;
    ...
    {
        doselist[i] = DoseBBLoader(m_Config.mImageInformation.sSourceFileMask,
                                   m_Config.mImageInformation.nFirstFileIndex+i,
                                   1.0f,
                                   fdarkBBdose); // D(I*n-Idc) in the doseBBroi
    }

    m_corrector.SetDoseList(doselist);

// here Exceptions need to be added to verify if the selected module is compatible with the number of loaded images
    switch (m_BBOptions)
    {
            ...
            if (nBBSampleCount!=0)
            {
                ...
                for (size_t i=0; (i<nBBSampleCount && (updateStatus(float(i)/nBBSampleCount,"Calculating ThinPlateSplines interpolation")==false)); i++)
                {
                    ...
                    else
                    {
                        ...
                        memcpy(bb_sample_parameters+i*(values_bb.size()+3),
                               temp_parameters,
                               sizeof(float)*(values_bb.size()+3));
                    }
                }
            }
...
kipl::base::TImage <float,2> BBLogNorm::BBExternalLoader(const std::string &fname,
                                                         const std::vector<size_t> &roi,
                                                         float &dose)
{
...
kipl::base::TImage <float,3> BBLogNorm::BBExternalLoader(const std::string &fname,
                                                         int N,
                                                         const std::vector<size_t> &roi,
                                                         int firstIndex,
                                                         std::vector<float> &doselist)
