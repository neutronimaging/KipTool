...
//    virtual int Configure(std::map<std::string, std::string> parameters); /// Configure all parameters and calls PrepareBBData
    /// Configure all parameters and calls SetRoi and PrepareBBData
    virtual int Configure(KiplProcessConfig config, std::map<std::string, std::string> parameters);

    /// Configure all parameters and does not call PrepareBBData
    virtual int ConfigureDLG(KiplProcessConfig config, std::map<std::string, std::string> parameters);

    virtual std::map<std::string, std::string> GetParameters();

    /// load all images that are needed for referencing in the current roi
    virtual void LoadReferenceImages(const std::vector<size_t> &roi);

    /// set the current roi to be processed and calls LoadReferenceImages
    ...
    /// computes and returns interpolation error and mask on OB image with BBs
    virtual float GetInterpolationError(kipl::base::TImage<float,2> &mask);

    /// read all data (entire projection) that I need and prepare them for the BB correction, it is now called in LoadReferenceImages
    virtual kipl::base::TImage<float, 2> GetMaskImage();

    virtual void PrepareBBData();

    /// load BB images pre-processed elsewhere
    virtual void LoadExternalBBData(const std::vector<size_t> &roi);
...
protected:
    ...
    bool bSameMask;                          ///< boolean value on the use of the same mask computed for OB images with
                                             ///< BB also for sample image with BB, in case of false the mask is recomputed
                                             ///< on the first available image with BB. This implies that exists an image
                                             ///< with the sample and without BB and the sample is in the exact same position
    ...
    std::vector<size_t> dose_roi;            ///< region of interest for dose coputation in projection images
    ...
    /// Loader function and dose computation for standard reference images (OB and DC)
    ...
    /// Loader function and dose computation for BB reference images (OB with BB and sample with BB)
    ...
    /// Loader function that only compute Dose in the BB dose roi , whitout loading the entire image
    virtual float DoseBBLoader(  std::string    fname,
                                 int            firstIndex,
                                 float          initialDose,
                                 float          doseBias);

    /// Loader function for externally created BB, open beam case (only 1 image)
    virtual kipl::base::TImage<float,2> BBExternalLoader(const std::string         &fname,
                                                         const std::vector<size_t> &roi,
                                                               float               &dose);

    /// Loader function for externally created BB, sample image case (nProj images with filemask)
    virtual kipl::base::TImage<float,3> BBExternalLoader(const std::string         &fname,
                                                               int                  N,
                                                         const std::vector<size_t> &roi,
                                                               int                  firstIndex,
    ...
    /// get the interpolation parameters for the polynomial case
    void PreparePolynomialInterpolationParameters();

    /// get the interpolation parameters for the thin plates spline case
    int  PrepareSplinesInterpolationParameters();
