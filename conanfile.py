...
class KipToolRecipe(ConanFile):
    ...
    def generate(self):
        ...
        self.framework_folder_KipTool = os.path.abspath(os.path.join(self.build_folder, self.cpp.build.bindir, 'KipTool.app', 'Contents', 'Frameworks'))
        ...
        if self.settings.os == "Windows":
            ...
            dst = os.path.join(self.framework_folder_KipTool,"../",'resources')
        ...
        if sys.version_info[1] > 9:
            shutil.copytree(
                os.path.join(self.source_folder,"applications","kiptool","resources"), 
                dst,
                dirs_exist_ok=True,
                )
        else:
    ...
    def build(self):
        ...
        if self.settings.os == "Macos":
            copy(self, "*.dylib", self.lib_folder, self.framework_folder_KipTool, excludes='*cpython*')
